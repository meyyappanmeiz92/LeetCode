...
/// <summary>
/// initialise result array with length same as nums1(given array)
/// do a for loop on nums1 array
/// initialise two variables such as max = -1 and k = 0
/// do inner for loop on nums2 array
/// check if nums[i] == nums[j]
/// if true assign k = j and break out of that loop
/// do another for loop such that j = k and j < nums2.Length
/// check if nums2[j] > nums1[i]
/// if true, assign max = nums2[j] and break out of that loop
/// out of that inner loop assign result[i] = max
/// after iterating over all the elements, return result
/// </summary>
